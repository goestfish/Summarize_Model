...
output_file_path = 'rated_summ.json'

REQUEST_LIMIT_PER_MINUTE = 10
SECONDS_PER_REQUEST = 60 / REQUEST_LIMIT_PER_MINUTE

def rate_article_with_gemini(title, summary_text):
    prompt = (
        ...
        f"Content:\n{summary_text}\n\n"
        f"Provide the response as a JSON object with two fields: 'rate1' and 'rate2'."
    )
...
try:
    for i, article in enumerate(articles):
        ...
        summary_text = article.get("generated_summary", "")

        if summary_text.strip():
            rate1, rate2 = rate_article_with_gemini(article["title"], summary_text)
